 ...
 * file the function also initializes the CoMaEventFile data structure
 * by calling openCoMaEventFile(). The access_mode parameter is
 * always READWRITE.
 */
 ...
 * the event file the numbering has to start at 1. So the routine adds
 * a 1 to the pixel index. The return value is the error status. */
int addCoMaEvent2File(CoMaEventFile* ef, CoMaEvent* event);

/** Read the next CoMaEvent from the CoMaEventFile.  This routine
 * increases the internal counter of the CoMaEventFile data structure.
 * In the event file the numbering of the pixels starts at 1, whereas
 * in the returned CoMaEvent data structure the numbering starts at 0.
 * The return value is the error status. The return value is the error
 * status. */
