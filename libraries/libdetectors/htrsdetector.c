...
int addImpact2HTRSDetector(HTRSDetector* hd, Impact* impact)
{
  ...
  if (charge > 0.) {
    ...
    if (INVALID_PIXEL != ring[0]) {
	  ...
	  (impact->time > hd->pixels.array[ring[0]][number[0]].reset_until)) {
	// The photon can be detected in this pixel.
	// The pixel is NOT within the shaping time after some previous event.
	...
	if ((event.pha>=hd->generic.pha_threshold) && 
	  ...
	  // Check if the pixel has to be reset (clear the charge).
	  if (hd->pixels.array[ring[0]][number[0]].charge > 350.) { // Threshold 350 keV
	    hd->pixels.array[ring[0]][number[0]].charge = 0.;
	    hd->pixels.array[ring[0]][number[0]].reset_until = 
	      impact->time + hd->shaping_time + hd->reset_time;
	  }
