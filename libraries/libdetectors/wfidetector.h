...
typedef struct {
   ...
   * In the current implementation this can be either 1 or 2. */
  int readout_directions;

  /** Time required to read out one line of the WIF detector (given in [s]). 
   * This is the time interval between the readout of two subsequent detector lines. */
  double line_readout_time;

  /** Time required to clear a line of pixels on the detector. (given in [s]). 
   * In the current implementation the clear time is a part of the readout time. */
  double line_clear_time;

  /** Current readout time (given in [s]). 
   * This is used as a counter for the current readout time of the WFIDetector,
   * i.e., the point of time, when the last line readout has been performed. */
  double readout_time;

  /** Current readout lines of the WFI detector. 
 ...
 * The routine is responsible to set up the initial the WFIDetector configuration which
 * is given in the WFIDetectorParameters data structure.
 ...
 * function checks, whether and which lines have to be read out.
 * In case it is necessary it calls the readoutLinesWFIDetector() function, which actually
 * performs the readout. */
 ...
 * of the WFI detector. 
 * This function implements the different readout modes of the WFIDetector model. 
 * According to the chosen number of readout lines it performs the readout of the currently 
 * active detector lines. 
 * The events read from the detector pixels are stored in the output even file. */
inline int readoutLinesWFIDetector(WFIDetector*);

/** Add a photon impact to the WFIDetector pixel array.
 * This is the standard routine to be called for the simulation of the WFIDetector.
 * For a new photon incident on the detector this routine determines the resulting
 * generated charge from the detector response, sums the charge to the affected 
 * detector pixels (one or several according to the selected split model),
 * and checks whether the detector has to be read out according to the chosen readout 
 * mode. In the latter case the right routines for the detector readout are called 
 * and the events are stored in the output event file. */
int addImpact2WFIDetector(WFIDetector*, Impact*);

/** Check the detector sensitivity in the given detector line at the specified time. 
 * This routine checks, whether the WFIDetector is sensitive in line 'y' at the given
 * time. According to the currently implemented readout strategy the WFIDetector is 
 * insensitive to new photons in an interval immediately after the line readout. This
 * interval is called 'line_clear_time'. 
 * This function checks whether the designated line was just read out and the time
 * lies within the clear time.
 * It returns 1 if the detector is sensitive in the line 'y' at the specified time. 
