// The X-ray telescope simulation can handle several source spectra. To each individual
// X-ray source a spectrum can be assigned. The background spectrum is also handled in 
// the same way as the source spectra.
// The simulation loads the spectra from PHA files containing the counts per 
// detector PHA channel. That means in order to create an appropriate input spectrm from
// a specified model, the energies have to be binned to the EBOUNDS of the detector
// response.
// The spectrum (i.e. the counts) can be normalized in an arbitrary way, as the simulation
// normalizes the spectrum to 1 after reading it from the file. The source count rate / flux
// has to be set via the 'src_cps' field in the source catalog.
...
int get_spectra(struct Spectrum_Store *, long Nchannels, char filenames[N_SPECTRA_FILES][FILENAME_LENGTH],
		int Nfiles, struct PSF_Store psf_store);
