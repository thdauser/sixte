...
/////////////////////////////////
int scan_tes_events_getpar(
			   char inputfile[],
			   double* time_unit,
			   int* units_before_pulse,
			   int* units_after_pulse
			   ) 
{
  char msg[MAXMSG];             // error output buffer
  int status = EXIT_SUCCESS;
  ...
  else if ((status = PILGetReal("time_unit", time_unit))) {
    sprintf(msg, "Error reading the detector intrinsic time unit!\n");
    HD_ERROR_THROW(msg,status);
  }

  else if ((status = PILGetInt("units_before_pulse", units_before_pulse))) {
    sprintf(msg, "Error reading the time units required before a "
	    "high precision event!\n");
    HD_ERROR_THROW(msg,status);
  }

  else if ((status = PILGetInt("units_after_pulse", units_after_pulse))) {
    sprintf(msg, "Error reading the time units required after a "
	    "high precision event!\n");
    HD_ERROR_THROW(msg,status);
  }
