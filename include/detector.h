...
// by its number.
inline struct Point2i htrs_get_pixel2icoordinates(int pixel, Detector*);
...
inline int htrs_get_line(struct Point2d point, double m, double dt, Detector*);


// Get the line indices of the lines from each of the 3 groups of lines that define
// the hexagonal pixel shape.
inline void htrs_get_lines(struct Point2d, Detector*, int* l);
