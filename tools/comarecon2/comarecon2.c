...
int comarecon2_main() {
  ...
  FreeReconArrayFits(recon_fits);
    ...
    while (0==EventFileEOF(&eventfile->generic)) {
      ...
      ea->EventArray[ea->rawx+(ea->naxis1)/2][ea->rawy+(ea->naxis2)/2]+=ea->charge;
    ...
    //testFitsImage1d(ReconImage1d, "recon1dTest.fits", Size1, Size2);

    //Get the 1d image of the event array:
    EventImage1d=SaveEventArray1d(ea, &status);
    //testFitsImage1d(EventImage1d, "event1dTest.fits", Size1, Size2);
