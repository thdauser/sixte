...
*					            'PROCESS' output keyword in _psh output FITS file renamed as 'PROC0'
*					            'PROCESS' output keyword in _trg output FITS file divided in two ones ('PROC0' generated by TRIGGER and 'PROC1'
*            					generated by PULSEGRADE)
*            					'ANNALS' output keyword renamed as 'MOD0'
*                   20/01/2015  Clobber input parameter added
*                               Replaced extension names (EUR-TRG->TRIGGER, EUR-PSH->PSGRADE) and variable (and parameter) names after pulseshape->pulsegrade renaming
***********************************************************************/
...
- psgFile: Name of the output FITS file
...
The results are written into an _psg output FITS file (RunGroupIDRunIDSubrunID_psg.fits) and also
...
* - If there are no pulses in _trg input FITS file (empty TRIGGER extension)
* 	- Create output FITS files (_psg.fits file) (call createPulseShapeFile)
*	- Create MOD0 in the TRIGGER file
...
* 		- Create an MOD0 in the TRIGGER file
...
int main (int argc, char **argv)
{
	...
	if ((eventcnt == 0) && (eventsz == 0))	// There are no pulses in the _trg input FITS file
	{
		...
		if (createPulseShapeFile())		// Pulsegrade file (*_psg.fits)
		{
		...
		if (fits_close_file(trgObject,&status) || fits_close_file(psgObject,&status))
		{
		...
		message = "The FITS file " + string(trgName) + " will have an empty extension PSGRADE";
		writeLog(fileRef,"Warning",verbosity,message);
	...
	// Write output keywords
	strcpy(keyname,"EVENTSZ");
	if (fits_write_key(psgObject,TINT,keyname,&one,comment,&status))
	{
	    message = "Cannot write keyword " + string(keyname) + " in file " + string(trgName);
	    EP_EXIT_ERROR(message,status);
	}
	...
	if (fits_close_file(trgObject,&status) || fits_close_file(psgObject,&status))
	{
	    message = "Cannot close file(s) " + string(trgName) + "/" + string(psgName);
	    EP_EXIT_ERROR(message,status);
	}
...
int initModule (int argc, char **argv)
{
	...
	inparam psgradePars[npars];
	int optidx =0, par=0, fst=0, ipar; 
	string message, task="pulsegrade";

	psgradePars[0].name = "trgFile";
    psgradePars[0].description = " TRG Input file name";
    psgradePars[0].defValStr = "a_trg.fits";
    psgradePars[0].type =  "char";
	psgradePars[0].ValStr = psgradePars[0].defValStr;

	psgradePars[1].name = "psgFile";
    psgradePars[1].description = "Output file name";
    psgradePars[1].defValStr = "a_psg.fits";
    psgradePars[1].type =  "char";
	psgradePars[1].ValStr = psgradePars[1].defValStr;
	
	psgradePars[2].name = "tauFALL"; 
    psgradePars[2].description = "Fall time of the pulses (seconds, [>0])";
    psgradePars[2].defValReal = 3.5E-4;
    psgradePars[2].type = "double";
    psgradePars[2].minValReal = 1.E-10;
    psgradePars[2].maxValReal = 1.E+10;
	psgradePars[2].ValReal = psgradePars[2].defValReal;

	psgradePars[3].name = "inTaus";
    psgradePars[3].description = "Times the fall time of the pulses to establish the inner interval for event grading [>0]";
    psgradePars[3].defValReal = 10.;
    psgradePars[3].type = "double";
    psgradePars[3].minValReal = 1.E-10;
    psgradePars[3].maxValReal = 1.E+10;
	psgradePars[3].ValReal = psgradePars[3].defValReal;

	psgradePars[4].name = "outTaus";
    psgradePars[4].description = "Times the fall time of the pulses to establish the outer interval for event grading [>0, >inTaus]";
    psgradePars[4].defValReal = 40.;
    psgradePars[4].type = "double";
    psgradePars[4].minValReal = 1.E-10;
    psgradePars[4].maxValReal = 1.E+10;	
	psgradePars[4].ValReal = psgradePars[4].defValReal;

	psgradePars[5].name = "nameLog"; 
    psgradePars[5].description = "Output log file name";
    psgradePars[5].defValStr = "psg_log.txt";
    psgradePars[5].type = "char";
	psgradePars[5].ValStr = psgradePars[5].defValStr;
	
	psgradePars[6].name = "verbosity"; 
    psgradePars[6].description = "Verbosity level of the output log file (in [0,3])";
    psgradePars[6].defValInt = 3;
    psgradePars[6].type = "int";
    psgradePars[6].minValInt = 0;
    psgradePars[6].maxValInt = 3;
	psgradePars[6].ValInt = psgradePars[6].defValInt;
	
	psgradePars[7].name = "clobber"; 
    psgradePars[7].description = "Re-write output files if clobber=yes";
    psgradePars[7].defValStr = "no";
    psgradePars[7].type = "char";
	psgradePars[7].ValStr = psgradePars[7].defValStr;
	
	// Define structure for command line options
	static struct option long_options[npars1];
	for (optidx = 0; optidx<npars; optidx++)
	{
		long_options[optidx].name= psgradePars[optidx].name.c_str();
	...
	while ((par=getopt_long(argc, argv,"",long_options, &optidx ))!= -1) // while there are still parameters in command line
	{
		...
		switch(par)
		{
			case 0:
				...
				{
					if (long_options[optidx].name == psgradePars[i].name.c_str())
					{
						if (psgradePars[i].type == "char") //save char value for par
						{
							psgradePars[i].ValStr = optarg;
						}
						else // check if numeric value
						{
							...
							{
								message = "Invalid value for input argument " + string(psgradePars[i].name);
								EP_PRINT_ERROR(message,EPFAIL);return(EPFAIL);
							}
							if (psgradePars[i].type == "int")
							{
								psgradePars[i].ValInt = atoi(optarg);
							}
							else
							{
								psgradePars[i].ValReal= atof(optarg);
							}
						}
						break;
					} // endif
	...
	if(commandLine == 0)
	{
		if (interactivePars(psgradePars,npars,task))
		{
	...
	for(int i=0;i<npars; i++)
	{
		if(psgradePars[i].name == "trgFile")
		{
			strcpy(trgName, psgradePars[i].ValStr.c_str());
		}
		else if(psgradePars[i].name == "psgFile")
		{
			strcpy(psgName, psgradePars[i].ValStr.c_str());
		}
		else if(psgradePars[i].name == "tauFALL")
		{
			tauFALL = psgradePars[i].ValReal;
		}
		else if(psgradePars[i].name == "inTaus")
		{
			inTaus = psgradePars[i].ValReal;
		}
		else if(psgradePars[i].name == "outTaus")
		{
			outTaus = psgradePars[i].ValReal;
		}
		else if(psgradePars[i].name == "nameLog")
		{
			strcpy(nameLog,psgradePars[i].ValStr.c_str());
		}
		else if(psgradePars[i].name == "verbosity")
		{
			verbosity = psgradePars[i].ValInt;
		}
		else if (psgradePars[i].name == "clobber")
		{
			strcpy(clobberStr, psgradePars[i].ValStr.c_str());
		...
		if (psgradePars[i].type == "int" &&
	        (psgradePars[i].ValInt < psgradePars[i].minValInt || 
	         psgradePars[i].ValInt > psgradePars[i].maxValInt))
		{
	    
			message = "Parameter name " + string(long_options[optidx].name) + " out of range: [" +
					boost::lexical_cast<std::string>(psgradePars[i].minValInt) + "," + boost::lexical_cast<std::string>(psgradePars[i].maxValInt) + "]";
			EP_PRINT_ERROR(message,EPFAIL);return(EPFAIL);
		}
		else if (psgradePars[i].type == "double" &&
	        (psgradePars[i].ValReal < psgradePars[i].minValReal || 
	        		psgradePars[i].ValReal > psgradePars[i].maxValReal))
		{
			message = "Parameter name " + string(long_options[optidx].name) + " out of range: [" +
					boost::lexical_cast<std::string>(psgradePars[i].minValReal) + "," + boost::lexical_cast<std::string>(psgradePars[i].maxValReal) + "]";
			EP_PRINT_ERROR(message,EPFAIL);return(EPFAIL);
		}
	}
...
/***** SECTION 4 ************************************************************
* createPulseShapeFile function: This function creates the structure of the _psg output FITS file
*
* - Create _psg output FITS file (if it does not exist yet)
* - Create extension PSGRADE
* - Create keywords
***************************************************************************/
int createPulseShapeFile ()
{
	...
	// Create output FITS file: If it does already exist -> check clobber
	// Create Pulsegrade file
	if ( fileExists(string(psgName)) && clobber==1)
	{
	      if (remove(psgName)){
		  message = "Input file (" + string(psgName) + ") already exists & cannot be deleted ("+string(strerror(errno))+")";
		  EP_PRINT_ERROR(message,EPFAIL); return(EPFAIL);
	      }
	}
	else if(fileExists(string(psgName)) && clobber==0)
	{
	      message = "Input file (" + string(psgName) + ") already exists, it must not be overwritten "; 
	      EP_PRINT_ERROR(message,EPFAIL); return(EPFAIL);
	}
	if(!fileExists(string(psgName)))
	{
	      if(fits_create_file(&psgObject, psgName, &status))
	      {
		  message = "Cannot create file " + string(psgName);
		  EP_PRINT_ERROR(message,status); return(EPFAIL);
	      }
	}
	
	message = "Create Pulse Shape Fits File: " + string(psgName);
	writeLog(fileRef,"Log", verbosity,message);
	...
	if (fits_open_file(&psgObject,psgName,READWRITE,&status))
	{
	    message = "Cannot open input file " + string(psgName);
	    EP_PRINT_ERROR(message,status);return(EPFAIL);
	}
	if (fits_create_tbl(psgObject, BINARY_TBL,0,0,tt,tf,tu,extname,&status))
	{
	    message = "Cannot crate table " + string(extname) + " in file " + string(psgName);
	    EP_PRINT_ERROR(message,status);return(EPFAIL);
	}
	if (fits_movnam_hdu(psgObject, ANY_HDU,extname, extver, &status))
	{
	    message = "Cannot move to HDU " + string(extname) + " in file " + string(psgName);
	    EP_PRINT_ERROR(message,status);return(EPFAIL);
	}
	...
	if (eventcnt < 0)
	{
	    message = "Legal values for EVENTCNT (PSGRADE) are integer numbers greater than or equal to 0";
	    writeLog(fileRef, "Error", verbosity, message);
	    EP_PRINT_ERROR(message,EPFAIL);return(EPFAIL);
	}
	if (fits_write_key(psgObject,TLONG,keyname,&eventcnt,comment,&status))
	{
	...
	strcpy(keyname,"ENERGY");
	if (fits_write_key(psgObject,TDOUBLE,keyname,&energy,comment,&status))
	{
	    message = "Cannot write keyword " + string(keyname) + " in " + string(psgName);
	    EP_PRINT_ERROR(message,status); return(EPFAIL);  
	}
	...
	string processoutpsg (string("PULSEGRADE") 		+ ' ' +
	string(trgName)		+ ' ' + string(psgName)	    	+ ' ' +
	...
	strcpy(keyvalstr,processoutpsg.c_str());
	if (fits_write_key_longwarn(psgObject,&status))
	{
		message = "Cannot write long warn in output file " + string(psgName);
		EP_PRINT_ERROR(message,status);return(EPFAIL);
	}
...
int inDataIteratorTrg (long totalrows, long offset, long firstrow, long nrows,int ncols, iteratorCol *cols, void *user_strct)
{
	...
	short *qualityP,*qualityPin;		// Vector of QUALITY column
	short *tailbefore, *tailbeforein;
	...
	qualityPin = (short *) fits_iter_get_array(&cols[3]);
	...
	tailbeforein = (short *) fits_iter_get_array(&cols[4]);
...
int pulseShape (gsl_vector *pulse, double tstart, double tend, double qual, double tail, double tstartprev, double tendprev, double tstartnext)
{
	...
	// Add Grade Column to TRIGGER FITS
 	...
 	strcpy(objTrg.nameTable,"TRIGGER");
