...
int main(int argc, char **argv)
{
	...
	strcpy(straux,"Quality");
	status = fits_iter_set_by_name(&cols[2], pshObjectA, straux, TSHORT, InputCol);
	if (status)
	{
	    message = "Cannot iterate in column " + string(straux) +" in " + string(pshNameA);
	    EP_EXIT_ERROR(message,status);
	...
	strcpy(straux,"Grade");
	status = fits_iter_set_by_name(&cols[3], pshObjectA, straux, TSHORT, InputCol);
	if (status)
	{
	    message = "Cannot iterate in column " + string(straux) +" in " + string(pshNameA);
	    EP_EXIT_ERROR(message,status);
	...
	if (optmode == 0)
	{
		...
		strcpy(straux,"Quality");
		status = fits_iter_set_by_name(&cols[2], pshObjectB, straux, TSHORT, InputCol);
		if (status)
		{
		    message = "Cannot iterate in column " + string(straux) +" in " + string(pshNameB);
		    EP_EXIT_ERROR(message,status);
		...
		strcpy(straux,"Grade");
		status = fits_iter_set_by_name(&cols[3], pshObjectB, straux, TSHORT, InputCol);
		if (status)
		{
		    message = "Cannot iterate in column " + string(straux) +" in " + string(pshNameB);
		    EP_EXIT_ERROR(message,status);
	...
	strcpy(keyvalstr,processoutHZG.c_str());
	if (fits_write_key_longwarn(pshObjectA,&status))
	{
	...
	{
		message = "Cannot write keyword " + string(keyname) + " in " + string(pshNameA);
	...
	if (fits_write_key(pshObjectA,TSTRING,keyname,keyvalstr,comment,&status))
	{
	...
	if (optmode == 0)
	{
		...
		strcpy(keyname,"PROC2");
		strcpy(keyvalstr,processoutHZG.c_str());
		if (fits_write_key_longwarn(pshObjectB,&status))
		{
		...
		strcpy(keyname,"MOD1");
		strcpy(keyvalstr,mod1.c_str());
		if (fits_write_key(pshObjectB,TSTRING,keyname,keyvalstr,comment,&status))
		{
...
int initModule(int argc, char **argv)
{
    ...
    holzgaussPars[7].description = "Verbosity level of the output log file (in [0,3])";
