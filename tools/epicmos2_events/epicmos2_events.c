...
int epicmos2_events_main()
{
  ...
  do { // Beginning of the ERROR handling loop (will at most be run once).
    ...
    int exists;
    fits_file_exists(par.EPICmos2EventList, &exists, &status);
    CHECK_STATUS_BREAK(status);
    if (0!=exists) {
      if (0!=par.clobber) {
	// Delete the file.
	remove(par.EPICmos2EventList);
      } else {
	// Throw an error.
	char msg[MAXMSG];
	sprintf(msg, "file '%s' already exists", par.EPICmos2EventList);
	SIXT_ERROR(msg);
	status=EXIT_FAILURE;
	break;
      }
    ...
    headas_chat(3, "create new EPIC-mos2 event list file '%s' ...\n",
		par.EPICmos2EventList);
    fits_create_file(&fptr, par.EPICmos2EventList, &status);
    CHECK_STATUS_BREAK(status);
    ...
    if (EXIT_SUCCESS!=status) {
      ...
      sprintf(msg, "could not create binary table for events "
	      "in file '%s'", par.EPICmos2EventList);
      SIXT_ERROR(msg);
      break;
    }
