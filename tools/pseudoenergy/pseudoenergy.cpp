...
* version 7.2.0     21/05/09    Corrected energy calculation in findEnergy
...
The calculated pseudoenergy is written in the _psh input/output FITS file (adding a new column Pseudoenergy)
...
- pshFile: Name of the _psh input/output FITS file which contains the characteristics of the found pulses
- TorF: Optimal filtering will be done in time (0) or in frequency (1)
- Hp_OForRS: Optimal filter (1) or running sum filter (0) for High-Res Primary pulses
- Mp_OForRS: Optimal filter (1) or running sum filter (0) for Medium-Res Primary pulses
- Ms_OForRS: Optimal filter (1) or running sum filter (0) for Medium-Res Secondary pulses
- Lp_OForRS: Optimal filter (1) or running sum filter (0) for Low-Res Primary pulses
- Ls_OForRS: Optimal filter (1) or running sum filter (0) for Low-Res Secondary pulses
- LrsT: Running sum length (in the RS filter case) in seconds
- LbT: Baseline averaging length (in the RS filter case) in seconds
...
* 		- Input/output _psh FITS file
* - Read input keywords and check their values
* - If there are no pulses in _trg input FITS file (empty EUR-TRG extension) => The task finishes
...
* - If production mode => Iterator for EUR-FLT in TRIGGER
...
* - Iterator for EUR-TRG in TRIGGER
...
* 		- Call findEnergy
*       - Call writeEnergy
* - Create ANNALS in PULSESHAPE file
* - Call writeKeywords
...
int main (int argc, char **argv)
{
	...
	writeLog(fileRef,"Log", verbosity,"Into Energy Resolution Task");
	...
	// Open  input/output _psh FITS file
	...
	{
	    message = "Cannot move to " + string(extname) + " in " + string(pshName);
	    EP_EXIT_ERROR(message,status);
	}
	message = "Open PulseFits: " + string(pshName);
	writeLog(fileRef,"Log", verbosity,message);

	strcpy(keyname,"PROCESS");
	if (fits_read_key_longstr(pshObject,keyname, &processin,comment,&status))
	{
	    message = "Cannot read key " + string(keyname) + " in " + string(pshName);
	    EP_EXIT_ERROR(message,status);
	}
	...
	if (eventcnt == 0)	// There are no pulses in _trg input FITS file (empty EUR-TRG extension)
	{
	...
	if (samprate <= 0)
	{
		message = "Legal values for SAMPRATE (EUR-TRG) are real numbers greater than 0";
		writeLog(fileRef, "Error", verbosity, message);
		EP_EXIT_ERROR(message,EPFAIL);
	}
	strcpy(keyname,"MODE");
	...
	if (mode == 0)		// Calibration mode
	{
		...
		strcpy(keyname,"EVENTCNT");
		if (fits_read_key(fltObject,TLONG,keyname, &eventcnt_in,comment,&status))
		{
		...
		if (eventcnt_in < 0)
		{
			message = "Legal values for EVENTCNT (EUR-FLT in _flt) are non negative integer numbers";
			writeLog(fileRef, "Error", verbosity, message);
			EP_EXIT_ERROR(message,EPFAIL);
		}
		strcpy(keyname,"NRMFCTR");
		if (fits_read_key(fltObject,TLONG,keyname, &normalizationFactor,comment,&status))
		{
		    message = "Cannot read key " + string(keyname) + " in " + string(fltName);
		    EP_EXIT_ERROR(message,status);
		}
		...
		{
			message = "Legal values for NRMFCTR (EUR-FLT in _flt) are real numbers greater than 0";
			writeLog(fileRef, "Error", verbosity, message);
			EP_EXIT_ERROR(message,EPFAIL);
		}
	}
	...
	// Create ANNALS in _psh FITS file
	...
	{
	    message = "Cannot write key " + string(keyname) + " in " + string(pshName);
	    EP_EXIT_ERROR(message,status);
	}

	// Write keywords in _psh FITS file
	if (writeKeywords ())
	{
	...
	// Close output FITS files
	if (fits_close_file(pshObject,&status))
	{
	    message = "Cannot close file " + string(pshName);
	    EP_EXIT_ERROR(message,status);
	}
	...
	/*for (int i=0;i<khauxNumber;i++){
		delete [] khaux[i].limitMin;
		delete [] khaux[i].limitMax;
	}
	delete [] khaux;*/
	...
	writeLog(fileRef,"OK", verbosity,"Energy resolution Module OK");
...
int initModule(int argc, char **argv)
{
	...
	inparam eresolPars[npars];
	int optidx =0, par=0, fst=0, ipar; 
	string task="pseudoenergy ";

	eresolPars[0].name = "inFile";
    eresolPars[0].description = "Input fits filename";
    eresolPars[0].defValStr = "a.fits";
    eresolPars[0].type =  "char";
	eresolPars[0].ValStr = eresolPars[0].defValStr;
	
	eresolPars[1].name = "trgFile";
    eresolPars[1].description = "TRIGGER fits filename";
    eresolPars[1].defValStr = "a_trg.fits";
    eresolPars[1].type =  "char";
	eresolPars[1].ValStr = eresolPars[1].defValStr;
	
	eresolPars[2].name = "fltFile";
    eresolPars[2].description = "FILTER fits filename (only in calibration mode)";
    eresolPars[2].defValStr = "a_flt.fits";
    eresolPars[2].type =  "char";
	eresolPars[2].ValStr = eresolPars[2].defValStr;
	
	eresolPars[3].name = "pshFile";
    eresolPars[3].description = "PULSESHAPE fits filename";
    eresolPars[3].defValStr = "a_psh.fits";
    eresolPars[3].type =  "char";
	eresolPars[3].ValStr = eresolPars[3].defValStr;
	
	eresolPars[4].name = "TorF";
    eresolPars[4].description = "Optimal filtering will be done in time (0) or in frequency (1)";
    eresolPars[4].defValInt = 0;
    eresolPars[4].type =  "int";
    eresolPars[4].minValInt = 0;
    eresolPars[4].maxValInt = 1;
	eresolPars[4].ValInt = eresolPars[4].defValInt;

	eresolPars[5].name = "Hp_OForRS";
    eresolPars[5].description = "Optimal filter (1) or running sum filter (0) for High-Res Primary pulses";
    eresolPars[5].defValInt = 1;
    eresolPars[5].type =  "int";
    eresolPars[5].minValInt = 0;
    eresolPars[5].maxValInt = 1;
	eresolPars[5].ValInt = eresolPars[5].defValInt;
	
	eresolPars[6].name = "Mp_OForRS";
    eresolPars[6].description = "Optimal filter (1) or running sum filter (0) for Mid-Res Primary pulses";
    eresolPars[6].defValInt = 1;
    eresolPars[6].type =  "int";
    eresolPars[6].minValInt = 0;
    eresolPars[6].maxValInt = 1;
	eresolPars[6].ValInt = eresolPars[6].defValInt;
	
	eresolPars[7].name = "Ms_OForRS";
    eresolPars[7].description = "Optimal filter (1) or running sum filter (0) for Mid-Res Secondary pulses";
    eresolPars[7].defValInt = 1;
    eresolPars[7].type =  "int";
    eresolPars[7].minValInt = 0;
    eresolPars[7].maxValInt = 1;
	eresolPars[7].ValInt = eresolPars[7].defValInt;
	
	eresolPars[8].name = "Lp_OForRS";
    eresolPars[8].description = "Optimal filter (1) or running sum filter (0) for Low-Res Primary pulses";
    eresolPars[8].defValInt = 1;
    eresolPars[8].type =  "int";
    eresolPars[8].minValInt = 0;
    eresolPars[8].maxValInt = 1;
	eresolPars[8].ValInt = eresolPars[8].defValInt;
	
	eresolPars[9].name = "Ls_OForRS";
    eresolPars[9].description = "Optimal filter (1) or running sum filter (0) for Low-Res Secondary pulses";
    eresolPars[9].defValInt = 1;
    eresolPars[9].type =  "int";
    eresolPars[9].minValInt = 0;
    eresolPars[9].maxValInt = 1;     
	eresolPars[9].ValInt = eresolPars[9].defValInt;
	
	eresolPars[10].name = "LrsT";
    eresolPars[10].description = "Running sum length (in the RS filter case) (seconds)";
    eresolPars[10].defValReal = 30.E-6;
    eresolPars[10].type = "double";
    eresolPars[10].minValReal = 0.;
    eresolPars[10].maxValReal = 10.;
	eresolPars[10].ValReal = eresolPars[10].defValReal;
	
	eresolPars[11].name = "LbT";
    eresolPars[11].description = "Baseline averaging length (in the RS filter case) (seconds)";
    eresolPars[11].defValReal = 1.E-3;
    eresolPars[11].type = "double";
    eresolPars[11].minValReal = 0.;
    eresolPars[11].maxValReal = 10.;
	eresolPars[11].ValReal = eresolPars[11].defValReal;

	eresolPars[12].name = "nameLog";
    eresolPars[12].description = "Output log file Name";
    eresolPars[12].defValStr = "ers_log.txt";
    eresolPars[12].type = "char";
	eresolPars[12].ValStr = eresolPars[12].defValStr;
	
	eresolPars[13].name = "verbosity";
    eresolPars[13].description = "Verbosity Level of the output log file";
    eresolPars[13].defValInt = 3;
    eresolPars[13].type = "int";
    eresolPars[13].minValInt = 0;
    eresolPars[13].maxValInt = 3;
	eresolPars[13].ValInt = eresolPars[13].defValInt;

	// Define structure for command line options
	static struct option long_options[npars1];
	for (optidx = 0; optidx<npars; optidx++)
	{
		long_options[optidx].name= eresolPars[optidx].name.c_str();
	...
	while ((par=getopt_long(argc, argv,"",long_options, &optidx ))!= -1) // while there are still parameters in command line
	{
		...
		switch(par)
		{
			case 0:
				...
				{
					if(long_options[optidx].name == eresolPars[i].name.c_str())
					{
						if(eresolPars[i].type == "char") //save char value for par
						{
						    eresolPars[i].ValStr = optarg;
						}
						else // check if numeric value
						{
						    ...
						    {
						    	message = "Invalid value for input argument " + string(eresolPars[i].name);
						    	EP_PRINT_ERROR(message,EPFAIL); return(EPFAIL);
						    }
						    if (eresolPars[i].type == "int")
						    {
						    	eresolPars[i].ValInt = atoi(optarg);
						    }
						    else
						    {
						    	eresolPars[i].ValReal= atof(optarg);
						    }
						}
						break;
					} // endif
	...
	if (commandLine == 0)
	{
		if (interactivePars(eresolPars,npars,task))
		{
	...
	for(int i=0;i<npars; i++)
	{
		if(eresolPars[i].name == "inFile")
		{
			strcpy(inName, eresolPars[i].ValStr.c_str());
		}
		else if(eresolPars[i].name == "trgFile")
		{
			strcpy(trgName, eresolPars[i].ValStr.c_str());
		}
		else if(eresolPars[i].name == "fltFile")
		{
			strcpy(fltName, eresolPars[i].ValStr.c_str());
		}
		else if(eresolPars[i].name == "pshFile")
		{
			strcpy(pshName, eresolPars[i].ValStr.c_str());
		}
		else if(eresolPars[i].name == "TorF")
		{
			TorF = eresolPars[i].ValInt;
		}
		else if(eresolPars[i].name == "Hp_OForRS")
		{
			Hp_OForRS = eresolPars[i].ValInt;
		}
		else if(eresolPars[i].name == "Mp_OForRS")
		{
			Mp_OForRS = eresolPars[i].ValInt;
		}
		else if(eresolPars[i].name == "Ms_OForRS")
		{
			Ms_OForRS = eresolPars[i].ValInt;
		}
		else if(eresolPars[i].name == "Lp_OForRS")
		{
			Lp_OForRS = eresolPars[i].ValInt;
		}
		else if(eresolPars[i].name == "Ls_OForRS")
		{
			Ls_OForRS = eresolPars[i].ValInt;
		}
		else if(eresolPars[i].name == "LrsT")
		{
			LrsT = eresolPars[i].ValReal;
		}
		else if(eresolPars[i].name == "LbT")
		{
			LbT = eresolPars[i].ValReal;
		}
		else if(eresolPars[i].name == "nameLog")
		{
			strcpy(nameLog,eresolPars[i].ValStr.c_str());
		}
		else if(eresolPars[i].name == "verbosity")
		{
			verbosity = eresolPars[i].ValInt;
		}
	  
		// Check if parameter value is in allowed range
		if (eresolPars[i].type == "int" &&
				(eresolPars[i].ValInt < eresolPars[i].minValInt ||
						eresolPars[i].ValInt > eresolPars[i].maxValInt))
		{
			message = "Parameter name " + string(long_options[optidx].name) + " out of range: [" +
					boost::lexical_cast<std::string>(eresolPars[i].minValInt) + "," + boost::lexical_cast<std::string>(eresolPars[i].maxValInt) + "]";
			EP_PRINT_ERROR(message,EPFAIL); return(EPFAIL);
		}
		else if ( eresolPars[i].type == "double" &&
				(eresolPars[i].ValReal < eresolPars[i].minValReal ||
						eresolPars[i].ValReal > eresolPars[i].maxValReal))
		{
			message = "Parameter name " + string(long_options[optidx].name) + " out of range: [" +
					boost::lexical_cast<std::string>(eresolPars[i].minValReal) + "," + boost::lexical_cast<std::string>(eresolPars[i].maxValReal) + "]";
			EP_PRINT_ERROR(message,EPFAIL); return(EPFAIL);
		}
	}
...
*                                the EUR-FLT extension in the _trg input FITS file and works iteratively
*
* - Declare variables
...
int inDataIteratorTrg (long totalrows, long offset, long firstrow, long nrows,int ncols, iteratorCol *cols, void *user_strct)
{
	...
	int *grade, *gradein;			// Vector of GRADE column
	...
	if (toGslVector(((void **)&dif),&difgsl,nrows,0,TDOUBLE))
	{
   	...
   	qualityin = (int *) fits_iter_get_array(&cols[4]);
	quality = &qualityin[1];
	...
	gradein = (int *) fits_iter_get_array(&cols[5]);
	grade   = &gradein[1];
...
/***** SECTION 10 ***************************************
* writeEnergy function: This function adds information (Pseudoenergy column) to the "EUR-PSH" extension of _psh input/output FITS file
***********************************************/
int writeEnergy ()
{
	char val2[256];
	...
	// Add Pseudoenergy Column to PSH FITS
	IOData obj;
...
/***** SECTION 11 ************************************************************
* writeKeywords function: This function writes keywords in "EUR-PSH" extension of _psh input/output FITS file
****************************************/
int writeKeywords ()
{
	int status = EPOK, extver=0;
	...
	string(trgName) 		+ ' ' + string(fltName)      + ' ' + string(pshName)      + ' ' + string(inName) + ' ' +
	string(str_TorF) 	    + ' ' +
	...
	string processoutPSH ((string) processin + ' ' + processout);
	...
	{
	    message = "Cannot update key " + string(keyname) + " in " + string(pshName);
	    EP_PRINT_ERROR(message,status); return(EPFAIL);
	}
