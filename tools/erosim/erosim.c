...
int get_telid_arf(float** cumulARF, float ph_ener, int nener, float* ener_lo,
		int ntel, int* status) {
...
float** new_cumulARF(long nener, int ntel, int* status) {
	...
	CHECK_NULL_RET(cumulARF, *status, "memory allocation for cumul ARF failed",
			NULL);

	int ii;
	for (ii = 0; ii < nener; ii++) {
		cumulARF[ii] = (float*) malloc(ntel * sizeof(float));
		CHECK_NULL_RET(cumulARF[ii], *status,
				"memory allocation for cumul ARF failed", NULL);
	}
...
int erosim_main() {
	...
	do { // Beginning of ERROR HANDLING Loop.
		...
		if (0 != strcmp(ucase_buffer, "STDOUT")) {
			...
			char msg[MAXMSG];
			sprintf(msg,
					"could not open file '%s' for output of progress status",
					par.ProgressFile);
		...
		arf7->LowEnergy = (float*) malloc(
				arf7->NumberEnergyBins * sizeof(float));
		arf7->HighEnergy = (float*) malloc(
				arf7->NumberEnergyBins * sizeof(float));
		...
		float** cumulARF = new_cumulARF(arf7->NumberEnergyBins, NUM_TELS,
				&status);
		CHECK_STATUS_BREAK(status);
		for (ii = 0; ii < arf7->NumberEnergyBins; ii++) {
			for (kk = 0; kk < NUM_TELS; kk++) {
				if (kk == 0) {
					cumulARF[ii][kk] = subinst[kk]->tel->arf->EffArea[ii]
							/ arf7->EffArea[ii];
				} else {
					cumulARF[ii][kk] = cumulARF[ii][kk - 1]
							+ subinst[kk]->tel->arf->EffArea[ii]
									/ arf7->EffArea[ii];
				}
			}
		}
		...
		// Get a GTI.
		gti = getGTIFromFileOrContinuous(par.GTIFile, par.TSTART,
				par.TSTART + par.Exposure, par.MJDREF, &status);
		CHECK_STATUS_BREAK(status);
		...
		if (strlen(photonlist_filename_template) > 0) {
			for (ii = 0; ii < 7; ii++) {
				...
				sprintf(photonlist_filename, photonlist_filename_template,
						ii + 1);
				plf[ii] = openNewPhotonFile(photonlist_filename, telescop,
						instrume, subinst[ii]->tel->arf->Filter,
						subinst[ii]->tel->arf_filename,
						subinst[ii]->det->rmf_filename, mjdref, 0.0, tstart,
						tstop, par.clobber, &status);
				CHECK_STATUS_BREAK(status);
			}
			CHECK_STATUS_BREAK(status);
		}
		...
		if (strlen(impactlist_filename_template) > 0) {
			for (ii = 0; ii < 7; ii++) {
				...
				sprintf(impactlist_filename, impactlist_filename_template,
						ii + 1);
				ilf[ii] = openNewImpactFile(impactlist_filename, telescop,
						instrume, subinst[ii]->tel->arf->Filter,
						subinst[ii]->tel->arf_filename,
						subinst[ii]->det->rmf_filename, mjdref, 0.0, tstart,
						tstop, par.clobber, &status);
		...
		for (ii = 0; ii < 7; ii++) {
			...
			elf[ii] = openNewEventFile(rawdata_filename, telescop, instrume,
					subinst[ii]->tel->arf->Filter,
					subinst[ii]->tel->arf_filename,
					subinst[ii]->det->rmf_filename, mjdref, 0.0, tstart, tstop,
					subinst[ii]->det->pixgrid->xwidth,
					subinst[ii]->det->pixgrid->ywidth, par.clobber, &status);
			CHECK_STATUS_BREAK(status);
		...
		for (ii = 0; ii < 7; ii++) {
			...
			patf[ii] = openNewEventFile(evtfile_filename, telescop, instrume,
					subinst[ii]->tel->arf->Filter,
					subinst[ii]->tel->arf_filename,
					subinst[ii]->det->rmf_filename, mjdref, 0.0, tstart, tstop,
					subinst[ii]->det->pixgrid->xwidth,
		...
		if (1 == ac->nentries) {
			...
			for (ii = 0; ii < 7; ii++) {
				if (NULL != plf[ii]) {
					fits_update_key(plf[ii]->fptr, TSTRING, "ATTITUDE",
							par.Attitude, "attitude file", &status);
				}
				if (NULL != ilf[ii]) {
					fits_update_key(ilf[ii]->fptr, TSTRING, "ATTITUDE",
							par.Attitude, "attitude file", &status);
				}
				fits_update_key(elf[ii]->fptr, TSTRING, "ATTITUDE",
						par.Attitude, "attitude file", &status);
				fits_update_key(patf[ii]->fptr, TSTRING, "ATTITUDE",
						par.Attitude, "attitude file", &status);
				CHECK_STATUS_BREAK(status);
			}
		}
		...
		for (ii = 0; ii < 7; ii++) {
			...
			value = subinst[ii]->det->rmf->FirstChannel
					+ subinst[ii]->det->rmf->NumberChannels - 1;
			fits_update_key(elf[ii]->fptr, TLONG, keystr, &value, "", &status);
			CHECK_STATUS_BREAK(status);
		...
		// CCD rotation angle.
		for (ii = 0; ii < 7; ii++) {
			float rotation_angle = subinst[ii]->det->pixgrid->rota * 180.
					/ M_PI;
			fits_update_key(elf[ii]->fptr, TFLOAT, "CCDROTA", &rotation_angle,
					"", &status);
			fits_update_key(patf[ii]->fptr, TFLOAT, "CCDROTA", &rotation_angle,
					"", &status);
			CHECK_STATUS_BREAK(status);
		}
		CHECK_STATUS_BREAK(status);
		...
		do {
			...
			do {
				...
				// Photon generation.
				Photon ph;
				int isph = phgen(ac, srccat, MAX_N_SIMPUT, t0, t1, par.MJDREF,
						par.dt, fov7, &ph, &status);
				CHECK_STATUS_BREAK(status);
				...
				ii = get_telid_arf(cumulARF, ph.energy, arf7->NumberEnergyBins,
						arf7->LowEnergy, NUM_TELS, &status);
				CHECK_STATUS_BREAK(status);
				...
				while ((unsigned int) ((ph.time - t0 + simtime) * 100.
						/ totalsimtime) > progress) {
					progress++;
					if (NULL == progressfile) {
		...
		for (ii = 0; ii < 7; ii++) {
			...
			if (GS_NONE != subinst[ii]->det->split->type) {
				...
				phpat(subinst[ii]->det, elf[ii], patf[ii], par.SkipInvalids,
						&status);
				//CHECK_STATUS_BREAK(status);
			} else {
				...
				copyEventFile(elf[ii], patf[ii],
						...
						subinst[ii]->det->threshold_pattern_up_keV, &status);
				//CHECK_STATUS_BREAK(status);
		...
		for (ii = 0; ii < 7; ii++) {
			if (p2p[ii] != NULL) {
				...
				pha2pi_correct_eventfile(patf[ii], p2p[ii],
						subinst[ii]->filepath, subinst[ii]->det->rmf_filename,
						&status);
				CHECK_STATUS_BREAK_WITH_FITSERROR(status);
			}
		}
