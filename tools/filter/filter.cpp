...
- If the production mode is selected, the proper matched filter to each valid pulse is chosen from the pulse templates
  ...
  of a new extension of the _trg input FITS file (EUR-FLT).
...
* 	- Close pulses templates library FITS file (EUR-LIB extension)
...
* 	- Close aoutput FITS file (_flt.fits)
...
*	- Call iteration function: inDataIteratorTr
...
*   	- Create the extension EUR-FLT in _trg.fits file
...
*				- Write the EUR-FLT extension in _trg FITS file
*			- elseif it is not a valid pulse:
*				- Write 0's in the corresponding row in the EUR-FLT extension in _trg FITS file
...
int main (int argc, char **argv)
{
	...
	else if (mode == 1)		// Production mode
	{
		...
		// In order to write in EUR-FLT (_trg.fits) matrix are necessary
		...
		if (fits_movnam_hdu(trgObject, ANY_HDU,extname, extver, &status))
		{
		    message = "Cannot move to HDU " + string(extname) +" in " + string(trgName) + " to add a new table EUR-FLT";
		    EP_EXIT_ERROR(message,status);
		}
		strcpy(extname,"EUR-FLT");		
		...
		for (int i=0; i<eventcnt ;i++)
		{
			...
			if (gsl_vector_get(qualitygsl,i) == 0)	// Neither truncated or saturated
			{
				if (((gradei == 1) && (Hp == 1)) || ((gradei == 21) && (Mp == 1)) || ((gradei == 22) && (Ms == 1)) || ((gradei == 31) && (Lp == 1)) || ((gradei == 32) && (Ls == 1)))
				{
					...
					// Write the EUR-FLT in _trg FITS file
						...
						for (int j=0;j<nonValidPulsesBeginning;j++)
						{
							...
							if (writeFitsComplex (obj,optimalfiltergsl_matrix0))
							{
							    message = "Cannot run routine writeFitsComplex to write " + string(obj.nameCol) +
							    		" column in EUR-FLT (non-valid pulse number " + boost::lexical_cast<std::string>(j) + ")";
							    EP_EXIT_ERROR(message,EPFAIL);
							}
					...
					{
					    message = "Cannot run routine writeFitsComplex to write " + string(obj.nameCol) + " column in EUR-FLT";
					    EP_EXIT_ERROR(message,EPFAIL);
					}
					...
					if (writeFitsSimple (obj,nrmfctrgsl))
					{
					    message = "Cannot run routine writeFitsSimple to write " + string(obj.nameCol) + " column in EUR-FLT";
					    EP_EXIT_ERROR(message,EPFAIL);
					}
			...
			//If a pulse is not a valid one (and in this case is not at the beginning), columns in EUR-FLT in the corresponding row are filled with 0's'
			if (gsl_vector_get(validPulses,i) == 0)
			{
				if (alreadyValidPulse == 1)
				{
					...
					if (writeFitsComplex (obj,optimalfiltergsl_matrix))
					{
					    message = "Cannot run routine writeFitsComplex to write " + string(obj.nameCol) + " column in EUR-FLT (valid pulses = 0)";
					    EP_EXIT_ERROR(message,EPFAIL);
					}
					...
					if (writeFitsSimple (obj,nrmfctrgsl))
					{
					    message = "Cannot run routine writeFitsSimple to write " + string(obj.nameCol) + " column in EUR-FLT (valid pulses = 0)";
					    EP_EXIT_ERROR(message,EPFAIL);
					}
		...
		strcpy(keyname,"ANNALS");
		strcpy(keyvalstr,annalsall.c_str());
...
int initModule(int argc, char **argv)
{
    ...
    filterPars[10].description = "Verbosity Level of the output log file";
...
{	
	...
	// Open pulse templates library file (EUR-LIB extension)
...
int createFilterFile ()
{
	...
	if (mode == 0)	//Calibration mode
	{
		...
		{
		    message = "Filter file " + string(fltName) + " already exists: it cannot be overwritten " ;
		    EP_PRINT_ERROR(message,EPFAIL); return(EPFAIL);
		}
	...
	else if (mode == 1)
	{
		...
		string processoutTRG ((string) processin + ' ' + processoutFLT);
		strcpy(keyname,"PROCESS");
...
int inDataIteratorTrg (long totalrows, long offset, long firstrow, long nrows, int ncols, iteratorCol *cols, void *user_strct)
{
	...
	qualityin = (int *) fits_iter_get_array(&cols[1]);
	quality = &qualityin[1];
	...
	gradein = (int *) fits_iter_get_array(&cols[2]);
	grade = &gradein[1];
...
/***** SECTION 12 ************************************************************
* writeLib: This function is used when the ENERGY of the pulses is not in the library FITS file (EUR-LIB extension).
...
int writeLib (double energyK, gsl_matrix *matchedfiltermatrix)
{
	...
	gsl_vector *tauriseout = gsl_vector_alloc(1);
	gsl_vector *taufallout = gsl_vector_alloc(1);
	gsl_vector *pretrigsout = gsl_vector_alloc(1);
	gsl_matrix *igsl_matrixout = gsl_matrix_alloc(1,eventsz);
	gsl_vector_set(energyout, 0, energyK);
	gsl_vector_set(estenergyout, 0, -1.0);
	gsl_vector_set(tauriseout, 0, -1.0);
	gsl_vector_set(taufallout, 0, -1.0);
	gsl_vector_set(pretrigsout, 0, -1.0);
	...
	strcpy(obj.unit,"sec");
	if (writeFitsSimple(obj, tauriseout))
	{
	    message = "Cannot create " + string(obj.nameCol) + " column";
	    EP_PRINT_ERROR(message,EPFAIL); return(EPFAIL);
	}
	...
	if (writeFitsSimple(obj, taufallout))
	{
	    message = "Cannot create " + string(obj.nameCol) + " column";
	    EP_PRINT_ERROR(message,EPFAIL);return(EPFAIL);
	}

	// Creating PRETRIGS column
	strcpy(obj.nameCol,"PRETRIGS");
	strcpy(obj.unit,"samples");
	if (writeFitsSimple(obj, pretrigsout))
	{
	    message = "Cannot create " + string(obj.nameCol) + " column";
	    EP_PRINT_ERROR(message,EPFAIL);return(EPFAIL);
	}
