...
int erosita_pattern_recombination_main() {
  ...
  do { // ERROR handling loop
    ...
    if (16==eventfile.columns) {
      strcat(template_filename, "erosita.window16.eventlist.tpl");
    } else if (1024==eventfile.columns) {
      strcat(template_filename, "erosita.full1024.eventlist.tpl");
    } else if (64==eventfile.columns) {
      strcat(template_filename, "erosita.labor64.eventlist.tpl");
    } else {
      status = EXIT_FAILURE;
