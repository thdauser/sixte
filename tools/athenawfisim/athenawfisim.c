...
int athenapwfisim_main() {
	...
	do { // Beginning of ERROR HANDLING Loop.
		...
		srccat[0] = loadSourceCatalog(par.Simput, subinst[0]->tel->arf,
				&status);
		CHECK_STATUS_BREAK(status);
		...
		if (strlen(par.Simput2) > 0) {
			...
			strtoupper(ucase_buffer);
			if (0 != strcmp(ucase_buffer, "NONE")) {
				srccat[1] = loadSourceCatalog(par.Simput2, subinst[0]->tel->arf,
						&status);
				CHECK_STATUS_BREAK(status);
			}
		}
		...
		if (strlen(par.Simput3) > 0) {
			...
			strtoupper(ucase_buffer);
			if (0 != strcmp(ucase_buffer, "NONE")) {
				srccat[2] = loadSourceCatalog(par.Simput3, subinst[0]->tel->arf,
						&status);
				CHECK_STATUS_BREAK(status);
			}
		}
		...
		if (strlen(par.Simput4) > 0) {
			...
			strtoupper(ucase_buffer);
			if (0 != strcmp(ucase_buffer, "NONE")) {
				srccat[3] = loadSourceCatalog(par.Simput4, subinst[0]->tel->arf,
						&status);
				CHECK_STATUS_BREAK(status);
			}
		}
		...
		if (strlen(par.Simput5) > 0) {
			...
			strtoupper(ucase_buffer);
			if (0 != strcmp(ucase_buffer, "NONE")) {
				srccat[4] = loadSourceCatalog(par.Simput5, subinst[0]->tel->arf,
						&status);
				CHECK_STATUS_BREAK(status);
			}
		}
		...
		if (strlen(par.Simput6) > 0) {
			...
			strtoupper(ucase_buffer);
			if (0 != strcmp(ucase_buffer, "NONE")) {
				srccat[5] = loadSourceCatalog(par.Simput6, subinst[0]->tel->arf,
						&status);
				CHECK_STATUS_BREAK(status);
			}
		}
		...
		if (strlen(photonlist_filename) > 0) {
			plf = openNewPhotonFile(photonlist_filename, telescop, instrume,
					subinst[0]->tel->arf->Filter, subinst[0]->tel->arf_filename,
					subinst[0]->det->rmf_filename, par.MJDREF, 0.0, par.TSTART,
		...
		if (strlen(impactlist_filename) > 0) {
			ilf = openNewImpactFile(impactlist_filename, telescop, instrume,
					subinst[0]->tel->arf->Filter, subinst[0]->tel->arf_filename,
					subinst[0]->det->rmf_filename, par.MJDREF, 0.0, par.TSTART,
					tstop, par.clobber, &status);
			CHECK_STATUS_BREAK(status);
		...
		for (ii = 0; ii < nchips; ii++) {
			...
			elf[ii] = openNewEventFile(rawdata_filename, telescop, instrume,
					subinst[0]->tel->arf->Filter, subinst[0]->tel->arf_filename,
					subinst[0]->det->rmf_filename, par.MJDREF, 0.0, par.TSTART,
					tstop, subinst[ii]->det->pixgrid->xwidth,
		...
		do {
			...
			do {
				...
				int isph = phgen(ac, srccat, MAX_N_SIMPUT, t0, t1, par.MJDREF,
						par.dt, subinst[0]->tel->fov_diameter, &ph, &status);
				CHECK_STATUS_BREAK(status);
		...
		for (ii = 0; ii < nchips; ii++) {
			phproj(subinst[ii], ac, patf[ii], par.TSTART, par.Exposure,
					&status);
			CHECK_STATUS_BREAK(status);
		}
		CHECK_STATUS_BREAK(status);
