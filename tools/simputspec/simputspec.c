...
int simputspec_main() 
{
  ...
  do { // Beginning of ERROR HANDLING Loop.
	...
	break;
      ...
      // Evaluate the spectral model and store the data in a temporary FITS file.
    ...
    // Add the spectra and write them to a SIMPUT file.
    ...
    remove(par.Outfile);
    saveSimputMissionIndepSpec(simputspec, par.Outfile, "SPECTRUM", 1, &status);
    CHECK_STATUS_BREAK(status);
