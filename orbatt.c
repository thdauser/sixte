			  ...
			  )
{
  ...
  long entry;                   // number of lines in the satellite catalog
  ...
  do {    // beginning of error handling loop
    ...
    *sat_catalog = (struct Telescope *) malloc(MAX_NORBIT_ENTRIES * sizeof(struct Telescope));
    if (*sat_catalog==NULL) {
      status = EXIT_FAILURE;
    ...
    // Read-in the orbit and attitude data from the FITS files and store it in satellite catalog array.
    // first open orbit file
    ...
    // read lines from orbit file subsequently
    for(counter=0, entry=0; (counter<nrows)&&(!status) ; counter++) {
      if (get_orbtbl_row(orbit_fptr, counter, &time, &sat_r, &sat_v, &status)) break;

      if (time >= t0) {
	// check, if we are already at the end of the orbit file:
	if (counter>=nrows) {
	  status=EXIT_FAILURE;
	  sprintf(msg, "Error: Not enough orbit data available for the specified period!");
	  HD_ERROR_THROW(msg,status);
	  break;
	}
    ...
    if (fits_open_file(&att_fptr, attitude_filename, READONLY, &status)) break;
    ...
    if (hdutype==IMAGE_HDU) {
      status=EXIT_FAILURE;
      sprintf(msg, "Error: FITS extension in attitude file '%s' is not a table but an image (HDU number: %d)\n", attitude_filename, hdunum);
      HD_ERROR_THROW(msg,status);
      break;
    }
	...
	if (counter>=nrows) {
	  status=EXIT_FAILURE;
	  sprintf(msg, "Error: Not enough attitude data available for the specified period!");
	  HD_ERROR_THROW(msg,status);
	  break;
	}
	...
	if ((*sat_catalog)[entry].time != time) {
	  status=EXIT_FAILURE;
	  sprintf(msg, "Error: time values not equivalent for orbit and attitude data: %lf vs. %lf!", (*sat_catalog)[entry].time, time);
	  HD_ERROR_THROW(msg,status);
	  break;
	}
...
//	printf("%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\t%lf\n", time, (*sat_catalog)[entry].nx.x, (*sat_catalog)[entry].nx.y, (*sat_catalog)[entry].nx.z, (*sat_catalog)[entry].ny.x, (*sat_catalog)[entry].ny.y, (*sat_catalog)[entry].ny.z, (*sat_catalog)[entry].nz.x, (*sat_catalog)[entry].nz.y, (*sat_catalog)[entry].nz.z);
